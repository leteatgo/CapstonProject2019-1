...
import com.openbravo.pos.sales.restaurant.leteatgo.MenuEditor;
...
public class BrowsableData implements ListModel {
    ...
    private MenuEditor m_menuEditor;
    ...
    public final int removeRecord(int index) throws BasicException {
        Object[] values = (Object[])getElementAt(index);
        if (values.length == 24) 
            m_menuEditor.deleteMenu(values);
    ...
    public final int updateRecord(int index, Object value) throws BasicException {
        ...
        if (values.length == 24) 
            m_menuEditor.updateMenu(values);
    ...
    public final int insertRecord(Object value) throws BasicException {   
        Object[] values = (Object[]) value;
        if (values.length == 24) 
            m_menuEditor.insertMenu(values);
            
            
        if (canInsertData() && m_saveprov.insertData(value) > 0) { 
            int newindex;
            if (m_comparer == null) {                //  정렬조건이 없으면
                // Anadimos el elemento indicado al final...
                newindex = m_aData.size();           //  가장 마지막 index에 신 메뉴 추가   
             } else {                                //  정렬조건이 존재한다면
                 // lo insertamos en el lugar adecuado
                newindex = insertionPoint(value);   //     정렬조건에 따라 insertionPoint 찾기  
             }
             m_aData.add(newindex, value);          //  정해진 index에 value추가
